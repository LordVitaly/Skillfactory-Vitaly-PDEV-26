...
def win_check(slots):  # Функция проверки победителя
    ...
    if slots[2] == slots[4] == slots[6]:
        return 1
    # Ничья - если превышено количество ходов (!!! Лучше - сделать проверку на отсутствие возможностей для других комбо)
    if turn > 9:
        return 2


def game_start():  # Функция начала игры
...
def game_restart():  # Функция повторной игры
    while True:  # Спрашиваем, пока не будет утвердительного ответа, либо пока игрок не решит окончить игру
...
print('''
        Игра "Крестики-нолики" v.0.5a (для двух игроков). Автор скрипта: Соколов Виталий (LordVitaly)
...
player1_score = 0
player2_score = 0
player_1 = "Игрок 1"  # Можно добавить ввод имени игроков (идея на следующую версию)
player_2 = "Игрок 2"  # Можно добавить ввод имени игроков (идея на следующую версию)
while True:
    p1fig = None
    p2fig = None
